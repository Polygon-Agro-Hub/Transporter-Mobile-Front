...
const Jobs: React.FC<JobsScreenProp> = ({ navigation }) => {
  ...
  const todoJobs = [
    { id: "01", name: "Mr. Kusal Dies", time: "8:00AM - 2:00PM", count: 2 , status:"Started"},
    { id: "02", name: "Mr. Kusal Dies", time: "2:00PM - 8:00PM", count: 1, status:"Pending" },
  ];
...
/>
      ...
      >
        ...
        >
          ...
          <Text className={`text-lg ${activeTab === "todo" ? "font-bold" : "font-medium"}`}>
            To Do
          </Text>
        </TouchableOpacity>
        ...
        >
          ...
          <Text className={`text-lg ${activeTab === "completed" ? "font-bold" : "font-medium"}`}>
            Completed
          </Text>
        </TouchableOpacity>
      </View>
      ...
      <ScrollView className="mt-6 px-5">
        {dataToShow.map((item, index) => (
          ...
          onPress={() => navigation.navigate("OrderDetails")}
            ...
            key={index}
            className={` rounded-xl px-5 py-2 mb-5 shadow-sm border flex-row justify-between items-center ${item.status ==="Started" && activeTab === "todo" ? "bg-[#FFFBEA] border-[#F7CA21] " : "bg-white border-[#A4AAB7] "}`}
          >
            <View>
            ...
            <View className={`w-7 h-7  ${activeTab === "todo" ? "bg-yellow-400 rounded-full" : "bg-[#F3F3F3] rounded-full"} justify-center items-center `}>
        ...
        {activeTab === "completed" && completedJobs.length === 0 && (
          <Text className="text-center text-gray-500 mt-10">No completed jobs</Text>
        )}
      </ScrollView>
    </View>
  );
