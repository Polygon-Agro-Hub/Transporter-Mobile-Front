...
import { View, Text, TextInput, TouchableOpacity, Image, KeyboardAvoidingView, Platform, ScrollView, Alert, BackHandler, ActivityIndicator , Modal, Animated} from "react-native";
...
const showModal = (title: string, message: string) => {
  ...
  const handleLogin = async () => {
    ...
    if (!empid && password) {
      showModal(
        "Sorry",
        "Employee ID is not allowed to be empty"
      );
      return false;
    }
...
if (!response.ok || !data.success) {
  ...
  if (message.includes("invalid password")) {
    ...
    // );
showModal("Invalid Password!", "Please check the Password and retry again.");
...
useEffect(() => {
  ...
  return (
    ...
    >
        ...
        <Image source={require("../assets/logo.webp")} className="w-auto h-[22%]" resizeMode="contain" />
                <Image source={require("../assets/truck.webp")} className="w-auto h-[60%]" resizeMode="contain" />
    ...
    <View className="flex-1">
      ...
      {/* Form Section */}
      <LinearGradient colors={["#323232", "#0E0E0E"]} 
             start={{ x: 0, y: 0 }}
            end={{ x: 1, y: 0 }}
   ...
   <View >
        ...
        <View className="flex-row items-center ml-4">
            ...
            placeholderTextColor={"#F6F9FF"}
          />
          <TouchableOpacity onPress={() => setSecureTextEntry(!secureTextEntry)} className="mr-4">
            <Entypo
...
>
  <View className="flex-1 bg-black/50 justify-center items-center">
    ...
    >
      ...
      <View className="absolute bottom-0 left-0 right-0 h-3  "
